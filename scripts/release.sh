...
release-notes --org mattermost --repo mattermost-oncall-handler --start-sha "${LATEST_RELEASE_NEXT_COMMIT_SHA}" --end-sha "${FUTURE_RELEASE_SHA}"  --output ./build/_output/docs/relnote.md --required-author "" --branch main
...
hub release create -d -F ./build/_output/docs/relnote.md -a ./build/_output/bin/mattermost-oncall-handler-linux-arm64 -a ./build/_output/bin/mattermost-oncall-handler-linux-amd64 -a ./build/_output/bin/mattermost-oncall-handler-darwin-amd64 -a ./build/_output/bin/mattermost-oncall-handler-darwin-arm64 ${GITHUB_REF_NAME}
rm -rf ./build/_output/
